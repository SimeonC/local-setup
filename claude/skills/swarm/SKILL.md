...
| `Explore` | default tier | shadow of the built-in read-only search role (body from `explorer`) |
| `general-purpose` | default tier | shadow of the built-in catch-all (body from `worker`) |
...
**Built-in shadows.** `Explore`, `Plan`, and `general-purpose` are stock roles
...
regenerates each:
- `Explore`←`explorer` and `general-purpose`←`worker` reuse an existing template
  body verbatim with only `name:` swapped (generated by `shadow_from`).
...
record (incl. the `Explore` shadow resolving to the default tier after restart),
...
- The agent registry loads at **session start**. A newly added or edited
  ...
  Same applies after re-running `setup_agents.sh`. A mid-session shadow will not
