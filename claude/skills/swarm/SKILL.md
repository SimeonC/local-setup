...
| Agent | Model | Use |
|---|---|---|
| `Explore` | default tier | search, reads, research (read-only tools; canonical delegation name) |
...
| `Plan` | `tablecheck-1-code-task` | rare top tier — design, multi-file architecture, implementation planning, chain-review orchestration (overrides built-in Plan) |
| `general-purpose` | default tier | built-in catch-all override (body from `worker`) |
...
enough for nearly everything. Promote to `specialist` or `Plan` only when
the task clearly warrants it — not by habit.

`subagent_type` is the agent's **role**, never a model ID. The roster above plus
the remaining stock roles (`claude`, `statusline-setup`, `claude-code-guide`)
are the valid values.

**Built-in role overrides.** `Explore`, `Plan`, and `general-purpose` are stock roles
that ship model-unpinned (`model: "inherit"`), so they silently ran on the
session default. Claude Code's last-wins precedence lets a user agent of the
same name fully replace a built-in (no field-level merge), so `setup_agents.sh`
regenerates each:
- `Explore` is the canonical delegation name; lowercase `explorer` is the
  source template and compatibility alias. Both use the same read-only role body.
- `general-purpose`←`worker` reuses the existing worker role body.
- `Plan` is a first-class template (`agent_templates/Plan.md`) carrying its own
  `name: Plan`, so the main loop generates it directly. It replaces the built-in
  read-only planner with our design/architecture tier — full write tools,
  running `tablecheck-1-code-task`.
...
record (including `Explore` resolving to the default tier after restart),
and via `claude -p --agent worker` at top level.
...
- The agent registry loads at **session start**. A newly added or edited
  ...
  Same applies after re-running `setup_agents.sh`. A mid-session role override
...
- Promoting to `specialist`/`Plan` when `worker` suffices
